...
class PlayState(GameState):
  ...
  def handle_input(self, events):
    for event in events:
      if event.type == pygame.KEYDOWN:
        if (event.key == pygame.K_UP or event.key == pygame.K_w) and self.snake.direction != (0, 1):
          self.snake.direction = (0, -1)
        elif (event.key == pygame.K_DOWN or event.key == pygame.K_s) and self.snake.direction != (0, -1):
          self.snake.direction = (0, 1)
        elif (event.key == pygame.K_LEFT or event.key == pygame.K_a) and self.snake.direction != (1, 0):
          self.snake.direction = (-1, 0)
        elif (event.key == pygame.K_RIGHT or event.key == pygame.K_d) and self.snake.direction != (-1, 0):
          self.snake.direction = (1, 0)
...
class GameOverState(GameState):
  ...
  def draw(self):
    ...
    over_text = self.title_font.render("GAME OVER", True, self.config["colors"]["text"])
    score_text = self.font.render(f"Final Score: {self.score}", True, self.config["colors"]["text"])
    restart_text = self.font.render("Press R to Restart or Q for Menu", True, self.config["colors"]["text"])

    over_rect = over_text.get_rect(center=(self.manager.width / 2, self.manager.height / 2 - 50))
    score_rect = score_text.get_rect(center=(self.manager.width / 2, self.manager.height / 2))
    restart_rect = restart_text.get_rect(center=(self.manager.width / 2, self.manager.height / 2 + 50))
