...
class Renderer:
    ...
    def __init__(self, screen, config, cell_width, cell_height, map_width, map_height):
        self.screen = screen
        self.config = config
        ...
        def _load_asset(filename, scale_factor=1.0):
        ...
        # Load Food Image
        self.apple_image = _load_asset("apple.png", 0.9)
        ...
        self.tail_assets = {
            (1, 0): _load_asset(
                "tail_left.png"
            ),  # Tail goes left (snake body is to the right)
            (-1, 0): _load_asset(
                "tail_right.png"
            ),  # Tail goes right (snake body is to the left)
        ...
        self.body_turns = {
            ...
            ((0, 1), (-1, 0)): _load_asset(
                "body_topleft.png"
            ),  # In from Down, Out to Left
            ((1, 0), (0, -1)): _load_asset(
                "body_topleft.png"
            ),  # In from Right, Out to Up
            # body_topright.png (L-shape from top-right)
            ((0, 1), (1, 0)): _load_asset(
                "body_topright.png"
            ),  # In from Down, Out to Right
            ((-1, 0), (0, -1)): _load_asset(
                "body_topright.png"
            ),  # In from Left, Out to Up
            # body_bottomleft.png (L-shape from bottom-left)
            ((0, -1), (-1, 0)): _load_asset(
                "body_bottomleft.png"
            ),  # In from Up, Out to Left
            ((1, 0), (0, 1)): _load_asset(
                "body_bottomleft.png"
            ),  # In from Right, Out to Down
            # body_bottomright.png (L-shape from bottom-right)
            ((0, -1), (1, 0)): _load_asset(
                "body_bottomright.png"
            ),  # In from Up, Out to Right
            ((-1, 0), (0, 1)): _load_asset(
                "body_bottomright.png"
            ),  # In from Left, Out to Down
        }
    ...
    def draw_snake(self, snake):
        ...
        for i, (x, y) in enumerate(snake.body):
            ...
            if image:
                rect = image.get_rect(
                    topleft=(x * self.cell_width, y * self.cell_height)
                )
    ...
    def draw_walls(self, walls):
        ...
        thickness = max(
            1, int(min(self.cell_width, self.cell_height) * self.config["wall"])
        )

        walls_set = set(walls)
        drawn_segments = (
            set()
        )  # keep track of already drawn centre pairs to avoid duplicates
    ...
    def draw_grid(self, map_width, map_height):
        ...
        grid_color = tuple(
            min(255, c + 10) for c in self._hex_to_rgb(self.config["colors"]["grid"])
        )
